...
for i=1:size(mean(D1),2)
    ...
    text(angle(i),area_2500(i), mean(D1(:,i)),text1,'HorizontalAlignment','center','VerticalAlignment','bottom',Interpreter="latex",FontSize=12);
end

stem3(angle,area_3300,mean(D2));
for i=1:size(mean(D2),2)
    text2=[num2str(round(mean(D2(:,i)),1))];
    text(angle(i),area_3300(i), mean(D2(:,i)),text2,'HorizontalAlignment','center','VerticalAlignment','bottom',Interpreter="latex",FontSize=12);
end

stem3(angle,area_4100,mean(D3));
for i=1:size(mean(D2),2)
    text3=[num2str(round(mean(D3(:,i)),1))];
    text(angle(i),area_4100(i), mean(D3(:,i)),text3,'HorizontalAlignment','center','VerticalAlignment','bottom',Interpreter="latex",FontSize=12);
end

view(3);
title("Distance between Static maximum and Dynamic Maximum on Sand",Interpreter="latex",FontSize=15)
...
view(3)
title("Difference between Static maximum and Dynamic Maximum on Sand",Interpreter="latex",FontSize=16)
